package com.example.alivia.navigation

import androidx.compose.foundation.layout.padding
import androidx.compose.material.icons.Icons
import androidx.compose.material.icons.filled.Favorite
import androidx.compose.material.icons.filled.Home
import androidx.compose.material3.ExperimentalMaterial3Api
import androidx.compose.material3.Scaffold
import androidx.compose.runtime.Composable
import androidx.compose.ui.Modifier
import androidx.navigation.compose.NavHost
import androidx.navigation.compose.rememberNavController
import com.example.alivia.ui.components.BottomNavigationBar
import com.example.alivia.ui.screens.DetailsScreen
import com.example.alivia.ui.screens.FavoritesScreen
import com.example.alivia.ui.screens.HomeScreen
import androidx.navigation.compose.composable
import com.example.alivia.models.stretchingList


// Classe representando os itens da Bottom Bar
sealed class BottomBarScreen(val route: String, val icon: @Composable () -> Unit, val label: String) {
    object Home : BottomBarScreen(
        route = "home",
        icon = { androidx.compose.material3.Icon(Icons.Default.Home, contentDescription = "Home") },
        label = "Home"
    )

    object Favorites : BottomBarScreen(
        route = "favorites",
        icon = { androidx.compose.material3.Icon(Icons.Default.Favorite, contentDescription = "Favorites") },
        label = "Favoritos"
    )
}
@ExperimentalMaterial3Api
@Composable
fun NavGraph(
    onSettingsClick: () -> Unit,
    onHelpClick: () -> Unit
) {
    val navController = rememberNavController()

    Scaffold(
        bottomBar = {
            BottomNavigationBar(navController = navController)
        }
    ) { innerPadding ->
        NavHost(
            navController = navController,
            startDestination = BottomBarScreen.Home.route,
            modifier = Modifier.padding(innerPadding)
        ) {
            // Tela Home
            composable(BottomBarScreen.Home.route) {
                HomeScreen(
                    onStretchingSelected = { stretching ->
                        navController.navigate("details/${stretching.name}")
                    },
                    onSettingsClick = onSettingsClick,
                    onHelpClick = onHelpClick
                )
            }
// Tela de Favoritos
            composable(BottomBarScreen.Favorites.route) {
                FavoritesScreen(
                    onStretchingSelected = { stretching ->
                        navController.navigate("details/${stretching.name}")
                    },
                    onFavoriteToggle = { stretching ->
                        stretching.isFavorite = !stretching.isFavorite
                    }
                )
            }

// Tela de Detalhes
            composable("details/{stretchingName}") { backStackEntry ->
                val stretchingName = backStackEntry.arguments?.getString("stretchingName")
                val selectedStretching= stretchingList.first { it.name == stretchingName }
                DetailsScreen(selectedStretching)
            }
        }
    }
}